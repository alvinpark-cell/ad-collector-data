...
async function updatePowerlinkInsight(settings) {
  ...
  for (const keyword of keywords) {
    ...
    if (!prevWeek) {
      ...
      const prompt = '다음은 네이버 파워링크(검색광고) 목록이야. 어떤 광고주들이 있고, 주로 어떤 내용(소구점)으로 광고하고 있는지 3~4문장으로 한국어로 요약해줘. 마크다운 없이 평문으로.';
      try {
    ...
    const prompt = '다음은 네이버 파워링크(검색광고)의 지난 주 대비 이번 주 변화(신규 진입/이탈 광고주)야. ' +
      '새로 진입한 광고주가 있다면 어떤 소구점(제목/설명 기반)으로 들어왔는지, 이탈한 광고주가 있다면 뭐였는지 3~5문장으로 한국어로 요약해줘. 마크다운 없이 평문으로.';
    try {
...
module.exports = { updatePowerlinkInsight };
