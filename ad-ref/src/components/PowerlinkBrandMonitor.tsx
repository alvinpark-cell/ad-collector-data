            ...
            <div key={brand} style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden' }}>
              <button onClick={() => toggle(brand)}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  ...
                  {noAds ? (
                    <span style={{ fontSize: '13px', color: 'var(--text-muted)', background: 'rgba(var(--text-muted-rgb),0.1)', padding: '2px 8px', borderRadius: '4px' }}>광고 미집행 중</span>
                  ) : (
                      ...
                      {[{ item: pc, label: 'PC' }, { item: mo, label: 'MO' }].map(({ item, label }) => (
                        <div key={label}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
                            ...
                            {item && <span style={{ marginLeft: 'auto', fontSize: '12px', color: 'var(--text-muted)' }}>새로고침 {item.refreshCount}회 · {new Date(item.collectedAt).toLocaleDateString('ko-KR')}</span>}
                          </div>
                          {!item || item.ads.length === 0 ? (
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                              {item.ads.map((ad, i) => (
                                <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', background: 'var(--bg-elevated)', border: '1px solid var(--border)', borderRadius: '8px', padding: '10px' }}>
                                  {ad.localImage && (
                                    ...
                                    {ad.sublinks && ad.sublinks.length > 0 && (
                                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: '4px' }}>
                                        {ad.sublinks.map((s, si) => (
                                          <span key={si} style={{ fontSize: '11px', color: 'var(--accent-text)', background: 'var(--accent-soft)', padding: '1px 6px', borderRadius: '3px' }}>{s.title}</span>
                                        ))}
                                      </div>
                                    )}
                                    {ad.landingUrl && (
